...
import type { TodoCreate } from "@boilerplate-typescript-todo/types";
...
const createMock = vi.mocked(todoApi.create);
...
describe("useCreateTodoMutation", () => {
  ...
  it("configures useMutation with correct mutationKey, mutationFn, and onSuccess", () => {
    ...
    renderHook(() => useCreateTodoMutation());
    ...
    expect(options.mutationKey).toEqual(todoKeys.mutation.create());
  ...
  it("mutationFn calls todoApi.create(data)", async () => {
    useMutationMock.mockReturnValue({} as ReturnType<typeof useMutation>);
